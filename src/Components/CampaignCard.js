...
export default class CampaignCard extends Component {
  render() {
      const {campaignName,moneyDonated,location, done}  = this.props
      return (
      <div style={Styles.myCampaignCard}>
        ...
        <div style={Styles.myCampaignCardContent}>
          <p style={Styles.myCampaignMoneyDonated}>Target 550.000.000 , hoàn thành 40%</p>
          <div style={Styles.myCampaignLocation}>
            ...
            <p style={Styles.myCampaignLocationText}>Đồng Nai</p>
          </div>
        </div>
        <button style={done?Styles.donateBtnDone:Styles.donateBtn}>{done?"Done":"Donate"}</button>
      </div>
    );
  }
}
